...
function ProjectCard({ project }: { project: Project }) {
  return <article className="project-card" id={project.id}>
    ...
    <div className="project-image"><img src={sitePath("results/" + project.image)} alt={project.alt} loading="lazy" /></div>
    <div className="project-body"><p className="project-focus">{project.focus}</p><h3>{project.title}</h3><p className="project-description">{project.description}</p>
      ...
      <div className="project-actions"><a className="button blue" href={sitePath("reports/" + project.pdf)} target="_blank" rel="noreferrer">View PDF report <span aria-hidden="true">↗</span></a><a className="download-link" href={sitePath("reports/" + project.pdf)} download>Download PDF <span aria-hidden="true">↓</span></a></div>
    </div>
  </article>;
}
...
export default function Portfolio({ section = "projects" }: { section?: "projects" | "labs" }) {
  ...
  return <main id="top">
    ...
    <header className="site-header">
      <a className="brand" href={sitePath()}><span className="brand-mark" aria-hidden="true">IC</span><span><strong>Analog Design</strong><small>Engineering portfolio</small></span></a>
      <nav className="main-tabs" aria-label="Portfolio sections"><a aria-current={section === "projects" ? "page" : undefined} className={section === "projects" ? "active" : ""} href={sitePath()}>Projects & challenges <span>04</span></a><a aria-current={section === "labs" ? "page" : undefined} className={section === "labs" ? "active" : ""} href={sitePath("labs/")}>Other labs <span>13</span></a></nav>
    </header>
    {section === "projects" ? <>
      <section className="hero" id="main-content"><div className="hero-inner"><div className="hero-copy"><p className="eyebrow">PRECISION REFERENCES / ANALOG AMPLIFIERS</p><h1>Analog IC design.<br /><span>From calculation<br className="desktop-break" /> to circuit.</span></h1><p>Two mini projects. Two design challenges. A focused collection of transistor-level designs, simulation results, and complete technical reports.</p><div className="hero-actions"><a className="button white" href="#mini-projects">Explore mini projects <span aria-hidden="true">↓</span></a><a className="hero-text-link" href="#design-challenges">Design challenges <span aria-hidden="true">→</span></a></div></div><div className="hero-figure"><div><span>FEATURED DESIGN / MP-02</span><span>CADENCE VIRTUOSO</span></div><img src={sitePath("results/folded-cascode-ota-schematic.png")} alt="Transistor-level schematic of the fully differential folded-cascode OTA" /><div className="hero-readout"><span><b>71.05 dB</b>Open-loop gain</span><span><b>57.00 MHz</b>Open-loop GBW</span><span><b>82.02°</b>Phase margin</span></div></div></div></section>
      ...
      {topics.filter((group) => filteredLabs.some((lab) => lab[3] === group)).map((group) => <section className="lab-group" key={group}><div className="lab-group-heading"><h2>{group}</h2><span>{filteredLabs.filter((lab) => lab[3] === group).length} reports</span></div>{filteredLabs.filter((lab) => lab[3] === group).map(([flow, number, title, , file]) => <a className="lab-row" href={sitePath("reports/" + flow.toLowerCase() + "/" + file)} target="_blank" rel="noreferrer" key={flow + number}><span className={"platform-badge " + flow.toLowerCase()}>{flow}</span><span className="lab-number">{number === "00" ? "Prelab" : "Lab " + Number(number)}</span><h3>{title}</h3><span className="pdf-label">PDF <span aria-hidden="true">↗</span></span></a>)}</section>)}
